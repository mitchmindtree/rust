...
impl<T> Rc<T> {
    ...
    pub fn try_unwrap(self) -> Result<T, Rc<T>> {
        if self.is_unique() {
            unsafe {
                let val = ptr::read(&*self); // copy the contained object
    ...
    /// Returns a mutable reference to the contained value if the `Rc` has unique ownership.
    ///
    ...
    /// ```
    /// let mut x = Rc::new(3u);
    /// *x.try_get_mut().unwrap() = 4u;
    /// assert_eq!(*x, 4u);
    /// let y = x.clone();
    /// assert!(x.try_get_mut().is_none());
    /// ```
    #[inline]
    #[experimental]
...
mod tests {
    ...
    fn try_unwrap() {
        ...
        let x = Rc::new(4u);
        let y = x.clone();
        assert_eq!(x.try_unwrap(), Err(Rc::new(4u)));
        let x = Rc::new(5u);
        let w = x.downgrade();
        assert_eq!(x.try_unwrap(), Err(Rc::new(5u)));
    }
